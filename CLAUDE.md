...
**Known warning, harmless:** builds print `deprecated: .Language.LanguageName was deprecated...` from the theme's `layouts/partials/i18nlist.html`. The fix (upstream commit `3a4bee8`) isn't in a tagged theme release yet — only on `master`, bundled with an untested Bootstrap version bump and other unrelated changes — so it's left alone until the theme cuts a release with just that fix. Doesn't affect the build.
...
`config.toml` defines a Hugo Cloud Deploy target (`[[deployment.targets]]`) pushing to an S3 bucket (`s3://wardtalks.com`) fronted by CloudFront, invalidated via `cloudFrontDistributionID`. Deploy with `hugo deploy`, using the `wardtalks` AWS profile (assumes `wardtalks-deploy`, scoped to S3 read/write/delete on this bucket plus CloudFront invalidation — see Infrastructure below). `hugo deploy` is a two-way sync, not just an upload: it deletes remote files with no local match, so the role needs `s3:DeleteObject`, not just `PutObject`/`GetObject`. Cache-control rules differ for hashed static assets (JS/CSS/fonts: 1 year) vs. images vs. HTML/XML/JSON (short-lived, gzip'd).
